/*
 程序名：dminingmysql.cpp 本程序是数据中心的公共功能模块，用于从mysql数据原表中抽取文件，生成xml文件
 作者： 王梓伊
*/
...
int ifieldcount;                      // strfieldname 和ifieldlen数组中有效字段的个数
...
// 从starg.incfilename 文件中获取已抽取数据的最大id
long imaxincvalue;      // 自增字段的最大值
bool readincfile();     // 从starg.incfilename文件中抽取已获取数据的最大id
...
int main(int argc, char *argv[])
{
    ...
    {
        logfile.Write("程序不在时间区内");
        return 0;
    }
    logfile.Write("程序在时间区内正常运行");
    ...
    // PActive.AddPInfo(starg.timeout,starg.pname); // 进程的心跳信息写入共享内存
...
bool readincfile()     // 从starg.incfilename文件中抽取已获取数据的最大id
{
    ...
    // 如果打开starg.incfilename文件失败，表示第一次运行程序，也不必返回失败
    // 也可能时文件丢了，那没办法，只能重新抽取
